<?php
    ...
    function option_update_table_for_content( $value ) {
        global $wpdb;
        $table_name  = $wpdb->prefix . 'optin_content';
        $wpdb->get_results( 'select * from ' . $table_name.' where id = ' . $value['iframeId'] );
        $count = $wpdb->num_rows;

        if ( $count != 0 ) {
            $wpdb->update( $table_name, array( 'contentbox_code' => stripslashes( $value['iframeCode'] ), 'contentbox_name' => stripslashes( $value['iframeName'] ) ), array( 'id' => $value['iframeId'] ) );

        } else {
            $wpdb->insert( $table_name, array( 'contentbox_code' => stripslashes( $value['iframeCode'] ), 'contentbox_name' => stripslashes( $value['iframeName'] ) ) );
        }
    }
    ...
    function optin_update_table_for_overlay( $value ) {
        ...
        if ( $count != 0 ) {
            $wpdb->update( $table_name, array( 'overlay_color' => stripslashes( $value['overlayBackground'] ),
                'button_text' => stripslashes( $value['overlayButtonText'] ),
                'overlay_text' => stripslashes( $value['overlayText'] ),
                'image_id' => stripslashes( $value['overlayImage'] ),
                'datenschutz' => stripslashes( $value['page'] ),
                'datenschutz_on_or_off' => stripslashes( $value['overlayButtonDatenschutz'] ),
                'image_size' => stripslashes( $value['imageSize'] ),
                'height' => stripslashes( $value['height'] ),
                'width' => stripslashes( $value['width'] ),
                'display_text' => stripslashes( $value['displayText'] )), array( 'id' => $value['iframeId'] ));
        } else {
            $wpdb->insert( $table_name, array( 'overlay_color' => stripslashes( $value['overlayBackground'] ),
                'id' => $value['iframeId'],
                'button_text' => stripslashes( $value['overlayButtonText'] ),
                'overlay_text' => stripslashes( $value['overlayText'] ),
                'image_id' => stripslashes( $value['overlayImage'] ),
                ...
                'height' => stripslashes( $value['height'] ),
                'width' => stripslashes( $value['width'] ),
                'display_text' => stripslashes( $value['displayText'] )));
        }
    }
